...
blue = (0,0,255)
longitude_lt = 114.264104
latitude_lt = 22.337759
longitude_rd = 114.262725
latitude_rd = 22.335668
pixel_length = 0.0
pixel_width = 0.0
...
while(counter < count_lines):
    ...
    roomlist = [item.replace('"','') for item in roomlist_before]
    #rate for latitude
    ...
    while row < pixel_width:
        while column < pixel_length:
            if rgb_im.getpixel((column,row)) == blue:
                ...
                print "found blue at pixel:(" + str(row) + "," + str(column) + ")" +", coordinate: ("+str(longitude_bluePoint)+","+str(latitude_bluePoint)+") at "+str(floor)+"th image"
